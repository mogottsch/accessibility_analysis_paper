...
We first start to analyze the optimal X-Minute city metric, that is the entry in the Pareto set for which the X-minute city metric is optimal.
...
\begin{table}
  ...
  \begin{center}
    \begin{tabular}{lrrrr}
      ...
      Car & 3.19 & 2.00 & 3.00 & 4.00 \\
      Combined & 11.51 & 7.25 & 10.29 & 14.33 \\
      Public Transport & 12.78 & 9.00 & 12.00 & 16.00 \\
...
These modes record average times ranging from 11 minutes and 30 seconds to 14 minutes, with walking being the least time-efficient mode at an average of 14 minutes and 5 seconds. 
...
\end{figure}
As we can see initially (for the most accessible hexagons to the left), public transport and walking are the same, but as we move to less accessible hexagons, public transport improves.
...
For the combined mode we see a pattern that roughly is equal to the addition of the public transport and bicycle sharing scenario.
...
We see that the improvements of the bicycle scenarios at the cost of \euro{1} are the largest, with an improvement of 1 minute and 40 seconds, and also the most cost-effective, with a value of 1.68 minutes per euro.
...
The combined scenario of bicycle sharing and public transport fails the least with a maximum of 36\% for the bank category, followed by public transport and bicycle sharing.
...
Therefore, we will only look at hexagons and category combinations where a journey is possible within the given timeframe for all sustainable scenarios.
...
However, remember that the cost of 19ct/min tries to capture fuel costs, repair costs, insurance, and tax, but not acquisition costs.
