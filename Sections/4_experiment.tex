...
Transitioning from the simplest form of mobility, the third scenario focuses on public transport. 
...
We use four different datasets to calculate the Pareto sets of the X-minute city metric for the different scenarios.
...
Lastly, we also use land use data to identify where residential areas are located to calculate the X-minute city metric only for these areas.
...
In GTFS, routes allow that, as they are much more a group of trips presented to the rider under the same name or identifier.
...
Additionally, we also remove circular trips, altogether.


\subsubsection{Bicycle Sharing}
\label{subs:bicycle_sharing}

Our bicycle-sharing data was retrieved from the NextBike API over one year.
...
We use the land use data from the CORINE Land Cover (CLC) project \shortcite{CORINELandCover} to identify the residential areas.
